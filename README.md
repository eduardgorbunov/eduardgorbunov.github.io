...
The website is intentionally independent of generated-builder assets and legacy framework bundles. Keep page and publication edits in the static HTML files, shared styles in `assets/theme/css/modern-academic.css`, and shared behavior in `assets/theme/js/`.
...
There is no build step for deployment: GitHub Pages serves the files in this repository directly. For a local preview, run a static server from the repository root:
...
Edit publication cards directly in `publications.html`. Update `index.html` or `research.html` separately when a paper also belongs in the news or selected-research sections. Run the local checks above before publishing; topic tags, mentoring markers, contribution notes, distinctions, and news wording remain manually reviewed.
