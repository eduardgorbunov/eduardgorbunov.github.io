(function () {
  ...
  function updateClearState() {
    ...
    var hasActiveFilter = typeFilter.value !== "all" || activeTags().length > 0 || Boolean(hasActiveSearch);
  ...
  function updateFilterSummary(selectedType, selectedActiveTags, searchQuery) {
    if (!filterSummary) {
      return;
    }
  ...
  function applyFilters(syncMode) {
    ...
    cards.forEach(function (card) {
      ...
      var show = typeMatches && tagsMatch && searchMatches(card, searchQuery);
    ...
    updateFilterSummary(selectedType, selectedActiveTags, searchQuery);
